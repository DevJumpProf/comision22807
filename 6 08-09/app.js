/* ## Math */

/* 
1 + 1 = 2
2 * 2 = 4
2 - 2 = 0
2 / 2 = 1 
*/

/* ### % Modulo*/ 



/* let modulo = 21 % 7 

document.write ( modulo) */

String


/* ### Math.pow */
/* 
Math.pow(2,2) = 4;
Math.pow(3,2) = 9;
Math.pow(3,3) = 27;
 */

/* ### Math.round , Math.floor, Math.ceil */
/* round` redondeará un número al número entero más cercano. */
/* Math.round(6.5) = 7;
Math.round(6.45) = 6; */
/* .floor` siempre redondeará un número al número entero más cercano hacia abajo. `. */
/* Math.floor(6.999) = 6; */
/* ceil` siempre se redondeará al número entero más cercano hacia arriba. */
/* Math.ceil(6.0001) = 7; */


/* ### .length */

let nombre = "Marcos"

/* document.write (nombre.length) */


// decime 
// crear un programa que pida:

/*  
por prompt el nombre a la persona
por prompt la edad a la persona

salude con un alert "hola nombre tu edad es: edad"

va a mostrar en el documento : la cantidad de letras de tu nombre sumado a tu edad da un resultado de : resultado */

/* let nombre2 = prompt ("decime tu nombre, please")
let edad = Number (prompt ("decime tu edad"))

alert ("hola " + nombre2 +  " tu edad es "+ edad)

let resultado = nombre2.length+edad */

/* document.write("la cantidad de letras de " + nombre2 + " sumado a tu edad da un resultado de " + resultado)
 */
/* document.write (`la cantidad de letras de ${nombre2} sumado a tu edad, da un resultado de ${resultado}`)
 */


/* ## Operadores de comparación  */
/* 1 > 2;     // false
2 < 3;     // true
10 >= 10;  // true
100 <= 1;  // false */

let num1 = 10
let num2 = "10"


/* let resultados = num1==num2 */
let resultados = num1===num2 
document.write(resultados)


// tarea
/* operadores de comparacion y su funcionalidad
&& and
|| or
!=
!==
 */




















